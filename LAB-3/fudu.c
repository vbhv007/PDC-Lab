...
void make_array(int arr[5][5], int a, int b){
	int i, j;
	for(i = 0; i < a; i++){
		for(j = 0; j < b; j++){
			arr[i][j] = 0;
...
int main(void){
	...
	make_array(mul, a, b);
	int trans1[a][b]; 
	make_array(trans1, a, b);
	int trans2[a][b]; 
	make_array(trans2, a, b);
	int add_thread[a][b]; 
	make_array(add_thread, a, b);
	int sub_thread[a][b];
	make_array(sub_thread, a, b);
	int mul_thread[a][b];
	make_array(mul_thread, a, b);
	int t_thread[a][b];
	make_array(t_thread, a, b);
